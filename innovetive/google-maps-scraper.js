...
(async () => {
    try {
        ...
        if (!bestMatch) {
            console.log('No reliable place match found');
            return emptyResult();
        }
    ...
    function findBestMatch(links, searchQuery) {
        ...
        const branchMatch = searchQuery.match(/[-\u2013\u2014]\s*([a-z0-9][a-z0-9\s.'-]+)$/i);
        const branchWords = branchMatch
            ? normalize(branchMatch[1]).split(' ').filter(w => w.length > 2 && !stopWords.has(w))
            : [];
        ...
        for (const link of links) {
            ...
            if (branchWords.length && !branchWords.every(word => labelNorm.includes(word) || labelWords.has(word))) {
                continue;
            }
