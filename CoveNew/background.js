...
async function findIframeAndInjectContentScript(tabId) {
    ...
    if (!iframeSrcResult) {
        // Fallback for non-iframe sources (e.g. Cove/Paylocity): inject into top frame.
...
async function scrapeAndGoToNext() {
    ...
    if (!clickedNext) {
        ...
        // ClearCompany pagination is client-side (no full tab reload), so continue manually.
        setTimeout(scrapeAndGoToNext, 2500);
    }
}
...
async function handleStartScraping(sendResponse) {
    ...
    // Non-iframe mode (Cove/Paylocity): start scraping directly.
    if (iframeFrameId === 0) {
