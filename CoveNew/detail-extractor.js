(() => {
    ...
    // Get hospital name (priority: DOM > title segment > JSON-LD)
    let hospitalName = domData.hospitalName || '';
    const blockedHospitalName = /^(?:united veterinary care|encore vet group|alliance animal health|cove animal health)\b/i;
    if (!hospitalName && positionTitle) {
        const titleParts = positionTitle.split(/\s*[-|]\s*/).map(s => s.trim()).filter(Boolean);
        const fromTitle = titleParts.find(part => /(?:animal\s+hospital|veterinary\s+(?:hospital|center|clinic|care|specialists?)|pet\s+(?:hospital|clinic|care)|emergency\s+(?:hospital|center|clinic)|oncology\s+service|specialty\s+hospital|referral\s+center|care\s+center)/i.test(part));
        if (fromTitle && !blockedHospitalName.test(fromTitle)) hospitalName = fromTitle;
    }
    if (!hospitalName && jsonLd?.hiringOrganization?.name) {
        const org = jsonLd.hiringOrganization.name;
        if (!blockedHospitalName.test(org)) hospitalName = org;
    }
    // If generic UVC name, try to find specific hospital in description
    if (!hospitalName || blockedHospitalName.test(hospitalName) || hospitalName.toLowerCase().includes('united veterinary care')) {
        ...
        if (hospitalMatch && !blockedHospitalName.test(hospitalMatch[1].trim())) hospitalName = hospitalMatch[1].trim();
    }
