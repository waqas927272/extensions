document.addEventListener('DOMContentLoaded', () => {
    ...
    function getDisplayJobId(job) {
        return (job?.jobId || '').replace(/^PETFOLK-/i, 'PFV-');
    }

    function normalizePetfolkNameValue(value = '') {
        return (value || '')
            .toLowerCase()
            .replace(/&/g, ' and ')
            .replace(/[^a-z0-9]+/g, ' ')
    ...
    function isPetfolkHospitalNameValue(value = '') {
        const normalized = normalizePetfolkNameValue(value);
        return normalized === 'petfolk' ||
            normalized === 'petfolk veterinary and urgent care' ||
            normalized.startsWith('petfolk veterinary and urgent care ');
    }

    function getPetfolkHospitalNameForDisplay(value = '') {
        const clean = (value || '').replace(/\s+/g, ' ').trim();
        if (!clean) return '';

        const suffixMatch = clean.match(/^petfolk\s+veterinary\s*(?:&|and)\s*urgent\s+care\s*[-–—]\s*(.+)$/i);
        if (suffixMatch?.[1]?.trim()) {
            return `${PETFOLK_HOSPITAL_NAME} - ${suffixMatch[1].trim()}`;
        }

        return isPetfolkHospitalNameValue(clean) ? PETFOLK_HOSPITAL_NAME : clean;
    }

    function getHospitalNameForJob(job) {
        if (!job) return '';
        const displayName = getPetfolkHospitalNameForDisplay(job.hospital || '');
        return displayName || (isPetfolkJob(job) ? PETFOLK_HOSPITAL_NAME : '');
    }

    function normalizePetfolkJobHospital(job) {
        if (!job) return job;

        const displayName = getPetfolkHospitalNameForDisplay(job.hospital || '');
        if (displayName && (isPetfolkJob(job) || isPetfolkHospitalNameValue(displayName))) {
            job.hospital = displayName;
        } else if (isPetfolkJob(job)) {
            job.hospital = PETFOLK_HOSPITAL_NAME;
        }

        return job;
    }

    function normalizeSavedPetfolkHospitals(jobs = []) {
        let changed = false;
        const normalizedJobs = jobs.map(job => {
            const before = job?.hospital || '';
            normalizePetfolkJobHospital(job);
            if ((job?.hospital || '') !== before) changed = true;
            return job;
        });
    ...
    function isGenericOrganizationHospitalName(hospitalName) {
        return /^(petfolk|mission pet health|united veterinary care|svp|southern veterinary partners)$/i.test((hospitalName || '').trim());
    }

    function isMissionPetHealthHospital(hospitalName) {
        return isPetfolkHospitalNameValue(hospitalName);
    }
    ...
    function extractBetterHospitalNameFromDescription(text, location = '', city = '', state = '') {
        ...
        const patterns = [
            ...
            /\b(Petfolk)\s+is\s+(?:seeking|hiring)\b/i,
            /\bAt\s+(Petfolk),\s+/i,
    ...
    function isPetfolkWebsite(website) {
        try {
            const host = new URL(website || '').hostname.replace(/^www\./i, '').toLowerCase();
            return host === 'petfolk.com' || host.endsWith('.petfolk.com');
        } catch (_) {
            return false;
        }
    }

    function isPetfolkExpectedJob(jobOrHospital) {
        if (typeof jobOrHospital === 'string') {
            return isPetfolkHospitalNameValue(jobOrHospital) || /\bpetfolk\b/i.test(jobOrHospital || '');
        }

        return isPetfolkJob(jobOrHospital) || isPetfolkHospitalNameValue(jobOrHospital?.hospital || '');
    }

    function addressMatchesExpectedHospitalBrand(hospitalName, addressData) {
        if (isPetfolkExpectedJob(hospitalName)) {
            const hasAnyAddressResult = !!(
            ...
            if (!hasAnyAddressResult) return true;

            const businessName = addressData?.businessName || '';
            const website = addressData?.website || '';
            const hasBrandSignal = !!(businessName || website);
            if (!hasBrandSignal && (addressData?.streetAddress || addressData?.zipCode || addressData?.fullAddress)) {
                return false;
            }

            return isPetfolkHospitalNameValue(businessName) || isPetfolkWebsite(website);
        }
    ...
    function getPetfolkHospitalSuffixFromName(hospitalName = '') {
        const clean = (hospitalName || '').replace(/\s+/g, ' ').trim();
        const suffixMatch = clean.match(/^petfolk\s+veterinary\s*(?:&|and)\s*urgent\s+care\s*[-\u2013\u2014]\s*(.+)$/i);
        return suffixMatch?.[1]?.trim() || '';
    }

    function getPetfolkOfficialAlias(value = '') {
        const aliases = {
            'atlanta': 'Grant Park',
            'charlotte': 'SouthPark',
            'dallas': 'North Dallas',
        ...
        return aliases[normalizeSimpleText(value)] || '';
    }

    function normalizePetfolkOfficialKey(value = '') {
        return normalizeSimpleText(value)
    ...
    function getPetfolkOfficialLocationUrl(locationRecord) {
        const dmaSlug = locationRecord?.dmaSlug || '';
        const slug = locationRecord?.slug || '';
        return dmaSlug && slug
            ? `https://petfolk.com/locations/${dmaSlug}/vet-clinics/${slug}`
            : 'https://petfolk.com/locations';
    }

    async function getPetfolkOfficialLocations() {
    ...
    function findPetfolkOfficialLocationMatch(job, officialLocations = []) {
        ...
        officialLocations.forEach(locationRecord => {
            ...
            const cityKey = normalizeCityForCompare(locationRecord.address?.city || '');
            if (!cityKey) return;
        ...
        const cityMatches = cityKey ? byCity.get(cityKey) || [] : [];
        return cityMatches.length === 1 ? cityMatches[0] : null;
    }

    function formatPetfolkOfficialStreetAddress(address = {}) {
    ...
    async function fetchPetfolkOfficialAddressForJob(job) {
    ...
    const URGENT_CARE_ROLE_SIGNAL_PATTERN = /\burgent\s+care\s+(?:veterinarian|vet|dvm|doctor|role|position|practice|medicine)\b|\b(?:veterinarian|vet|dvm|doctor)\s+(?:for|in|with|on)?\s*urgent\s+care\b|\b(?:experience|experienced|background|worked|working)\b[^.\n]{0,160}\burgent\s+care\b|\burgent\s+care\b[^.\n]{0,160}\b(?:experience|experienced|required|requirement|preferred|candidate|veterinarian|vet|dvm|doctor)\b/i;
    const EMERGENCY_SIGNAL_PATTERN = /\bemergency\s+(?:care|veterinarian|vet|dvm|doctor|medicine|hospital|clinic|center|service|services)\b|\b(?:er|e\/r)\s+(?:veterinarian|vet|dvm|doctor|medicine)\b|\b(?:veterinarian|vet|dvm|doctor)\s+(?:for|in|with|on)?\s*(?:er|e\/r|emergency)\b|\b(?:experience|experienced|background|worked|working)\b[^.\n]{0,160}\b(?:emergency|er|e\/r)\s+(?:care|medicine|veterinarian|vet|dvm|doctor)\b/i;
    ...
    function hasUrgentCareAreaSignal(title = '', descriptionText = '', hospitalName = '') {
        const combined = `${title || ''}\n${hospitalName || ''}\n${descriptionText || ''}`;
        if (URGENT_CARE_SIGNAL_PATTERN.test(title || '')) return true;
        if (URGENT_CARE_ROLE_SIGNAL_PATTERN.test(combined)) return true;

        const requirementText = extractCandidateRequirementSection(descriptionText);
        if (requirementText && URGENT_CARE_SIGNAL_PATTERN.test(requirementText)) return true;

        return false;
    }

    function hasEmergencySignal(title, hospitalName = '', extraText = '') {
        if (EMERGENCY_SIGNAL_PATTERN.test(title || '')) return true;
    ...
    function getAOPFromTitle(title) {
        ...
        // Emergency
        if (hasEmergencySignal(t)) return 'Emergency Care';
    ...
    // ============ PETFOLK-SPECIFIC EXTRACTION ============

    function getTopLevelMetadataField(text, labels) {
        const candidates = Array.isArray(labels) ? labels : [labels];
        const source = text || '';
    ...
    function isPetfolkJob(job) {
        return /petfolk/i.test(`${job?.hospital || ''} ${job?.link || ''} ${job?.description || ''}`);
    }

    function extractRipplingJobIdFromLink(link = '') {
        const match = String(link).match(/\/petfolk\/jobs\/([^/?#]+)/i);
        return match ? match[1] : '';
    }

    function normalizePetfolkLocationLabel(value = '') {
        return (value || '')
            .replace(/\([^)]*\bcoming\s+soon\b[^)]*\)/gi, ' ')
            .replace(/\([^)]*\)/g, ' ')
            .replace(/\b.+?\s+area\s+of\s+(.+)$/i, '$1')
            .replace(/\bgreater\s+(.+?)\s+area$/i, '$1')
            .replace(/\s+/g, ' ')
            .trim();
    }
    ...
    function parseLocationValue(value) {
        ...
        const cityState = clean.match(/^(.+?),\s*([A-Za-z]{2}|[A-Za-z][A-Za-z\s]+)$/);
        if (cityState) {
            const city = normalizePetfolkLocationLabel(cityState[1].trim());
            const state = formatStateForStorage(cityState[2].trim());
            const stateAbbr = getStateAbbreviation(state);
            ...
            return {
                ...
                state,
                stateAbbr,
                location: stateAbbr ? `${formatCityForStorage(city)}, ${stateAbbr}` : `${formatCityForStorage(city)}, ${state}`
            };
        }

        const hybrid = clean.match(/^Hybrid\s*\(([A-Z]{2}),\s*US\)$/i);
        if (hybrid) {
            const state = formatStateForStorage(hybrid[1]);
            return {
                city: '',
                state,
                stateAbbr: getStateAbbreviation(state),
                location: clean
            };
        }

        const locationLabel = normalizePetfolkLocationLabel(clean);
        if (!locationLabel) return null;
        return { city: formatCityForStorage(locationLabel), state: '', stateAbbr: '', location: locationLabel };
    }

    function splitStoredLocationValues(value = '') {
        return (value || '')
            .split('|')
            .map(part => part.trim())
            .filter(Boolean);
    ...
    function extractLocationsFromText(text = '') {
        ...
        const addLocation = (raw) => {
            const parsed = parseLocationValue(raw);
            if (!parsed?.location) return;
            const key = normalizeSimpleText(parsed.location);
        ...
        const isLocationListLine = (line = '') => {
            if (!line || line.length > 80) return false;
            if (/:/.test(line)) return false;
            if (/^(description|requirements|additional qualifications|benefits|we believe|what you|lead |ownership|focus |part of|profit sharing)\b/i.test(line)) return false;
            if (/^(at petfolk|as a petfolk|petfolk provides|ideal candidates|our team enjoys|doctorate in|ability to|passion for|compassionate|life long|desirable compensation|state-of-the-art|no on-call|generous paid|professional liability|fear free|discount on)/i.test(line)) return false;
            return /[A-Za-z]/.test(line);
        };
        const addLocationLines = (block, stateAbbr = '') => {
            (block || '')
                .split(/\r?\n/)
                .map(normalizeListLine)
                .filter(isLocationListLine)
                .forEach(city => addLocation(stateAbbr && !/,/.test(city) ? `${city}, ${stateAbbr}` : city));
        };

        const locationsSection = text.match(/Locations:\s*\n((?:\s*-\s*[^\n]+\n?)+)/i);
        if (locationsSection) {
            addLocationLines(locationsSection[1]);
        }

        const formattedLocationSection = text.match(/^\s*\d+\s+(?:.+?\s+)?Locations:\s*\n([\s\S]*?)(?=^\s*(?:Requirements|Additional Qualifications|Benefits|Description|We believe)\b|(?![\s\S]))/im);
        if (formattedLocationSection) {
            const stateAbbr = inferStateAbbreviationFromText(text);
            addLocationLines(formattedLocationSection[1], stateAbbr);
        }
    ...
    function getJobLocations(job) {
        ...
        const seen = new Set();
        const addLocation = (raw) => {
            const parsed = typeof raw === 'string' ? parseLocationValue(raw) : raw;
            if (!parsed?.location) return;
            const key = normalizeSimpleText(parsed.location);
            if (seen.has(key)) return;
            seen.add(key);
        ...
        (Array.isArray(job.petfolkSourceLocations) ? job.petfolkSourceLocations : []).forEach(addLocation);
        (Array.isArray(job.locations) ? job.locations : []).forEach(addLocation);
        splitStoredLocationValues(job.allLocations || '').forEach(addLocation);
        splitStoredLocationValues(job.location).forEach(addLocation);
        extractLocationsFromText(getPetfolkSourceDescription(job)).forEach(addLocation);

        if (!locations.length && (job.city || job.state)) {
            addLocation(`${job.city || ''}${job.city && job.state ? ', ' : ''}${job.state || ''}`);
        }

        return locations;
    }

    function collectPetfolkLocations(job, sources = []) {
        const locations = [];
        const seen = new Set();
        const addLocation = (raw) => {
            const parsed = typeof raw === 'string' ? parseLocationValue(raw) : raw;
        ...
        sources.forEach(source => {
            ...
            if (Array.isArray(source)) {
                source.forEach(addLocation);
                return;
            }
            splitStoredLocationValues(source).forEach(addLocation);
        });

        return locations;
    }

    function getPetfolkSourceDescription(job) {
        return job?.petfolkSourceDescription || job?.description || '';
    }

    function getPetfolkLocationsForExtraction(job) {
        const storedLocations = collectPetfolkLocations(job, [
            job.allLocations || '',
            Array.isArray(job.petfolkSourceLocations) ? job.petfolkSourceLocations : [],
            Array.isArray(job.locations) ? job.locations : [],
            /\|/.test(job.location || '') ? job.location : ''
        ]);
        if (storedLocations.length > 1) return storedLocations;

        const descriptionLocations = extractLocationsFromText(getPetfolkSourceDescription(job));
        if (descriptionLocations.length > 1) return descriptionLocations;

        if (storedLocations.length === 1) return storedLocations;
        if (descriptionLocations.length === 1) return descriptionLocations;

        const currentLocation = collectPetfolkLocations(job, [job.location || '']);
        if (currentLocation.length) return currentLocation;

        return collectPetfolkLocations(job, [
            `${job.city || ''}${job.city && job.state ? ', ' : ''}${job.state || ''}`
        ]);
    }

    function normalizeSalaryAmount(rawAmount, suffix = '') {
        const numeric = parseFloat(String(rawAmount || '').replace(/[$,\s]/g, ''));
        if (!Number.isFinite(numeric)) return null;
        const multiplier = /k/i.test(suffix) ? 1000 : /m/i.test(suffix) ? 1000000 : 1;
    ...
    function extractPetfolkSalary(text = '') {
        ...
        for (const pattern of contextPatterns) {
            ...
            if (match[3] && /^[\d,]+(?:\.\d+)?$/.test(match[3]) && /(?:-|–|—|to)/.test(match[0])) {
                const min = normalizeSalaryAmount(match[1], match[2]);
                const max = normalizeSalaryAmount(match[3], match[4]);
                if (!min || !max || Math.max(min, max) < 1000 && !/\b(hour|hr|shift|day)\b/i.test(match[0])) continue;
                const unit = normalizeSalaryUnit(match[5] || match[0]);
                return `${formatSalaryValue(Math.min(min, max))}-${formatSalaryValue(Math.max(min, max))} ${unit}`;
            }

            const value = normalizeSalaryAmount(match[1], match[2]);
            if (!value || value < 1000 && !/\b(hour|hr|shift|day)\b/i.test(match[0])) continue;
            const unit = normalizeSalaryUnit(match[3] || match[0]);
            return `${formatSalaryValue(value)} ${unit}`;
        }
    ...
    function normalizeSalaryUnit(value = '') {
        if (/hour|hr/i.test(value)) return 'hourly';
        if (/shift/i.test(value)) return 'per shift';
        if (/\bday\b/i.test(value)) return 'daily';
        return 'yearly';
    }

    function extractPetfolkExperience(text = '') {
        const source = text || '';
        const requirementMatch = source.match(/Requirements:?([\s\S]*?)(?:Additional Qualifications|Benefits|We believe|$)/i);
        const searchable = requirementMatch ? requirementMatch[1] : source;
        const matches = [];

        const patterns = [
            /\b(\d+)\s*\+\s*(years?|yrs?)\b/gi,
            /\b(\d+)\s*(?:-|–|—|to)\s*(\d+)\s*(years?|yrs?|months?|mos?)\b/gi,
            /\b(\d+)\s*\+\s*(months?|mos?)\b/gi,
        ...
        for (const pattern of patterns) {
            ...
            while ((match = pattern.exec(searchable)) !== null) {
                if (/CE requirements|24\/7|100%|116M|2026/i.test(match[0])) continue;
                let value = '';
                if (match[2] && /^\d+$/.test(match[2])) {
                    value = `${match[1]}-${match[2]} ${normalizeExperienceUnit(match[3])}`;
                } else if (/\+/.test(match[0])) {
                    value = `${match[1]}+ ${normalizeExperienceUnit(match[2])}`;
                } else {
                    value = `${match[1]} ${normalizeExperienceUnit(match[2])}`;
                }
    ...
    function normalizeExperienceUnit(unit = '') {
        return /month|mo/i.test(unit) ? 'months' : 'years';
    }

    function extractPetfolkJobType(title = '', text = '') {
        const combined = `${title || ''} ${text || ''}`;
        if (/part[-\s]?time|weekend|relief|locum/i.test(combined)) return 'Part-Time';

        const employmentType = getTopLevelMetadataField(text, 'Employment Type');
        if (/CONTRACTOR/i.test(employmentType)) return 'Part-Time';
        if (/FT|FULL/i.test(employmentType)) return 'Full-Time';

        return 'Full-Time';
    }

    function determinePetfolkAreaOfPractice(title = '', text = '') {
        const combined = `${title || ''} ${text || ''}`.toLowerCase();
        if (/relief|locum|petfolkflex|gp\/uc\/er/.test(combined)) {
            return 'General Practice Care / Urgent Care / Emergency Care';
        }

        if (hasUrgentCareAreaSignal(title, text, PETFOLK_HOSPITAL_NAME)) {
            return 'Urgent Care';
        }

        return getAOPFromTitle(title) || 'General Practice Care';
    }

    function determinePetfolkPosition(title = '') {
        const t = (title || '').toLowerCase();
        if (/regional medical partner|regional medical director/.test(t)) return 'Medical Director';
        if (/partner veterinarian/.test(t)) return 'Partner Veterinarian';
        if (/veterinarian|dvm|locum|relief/.test(t)) return 'Associate Veterinarian';
        return getPositionFromTitle(title) || '';
    }

    function isRegionalMedicalPartnerJob(jobOrTitle) {
        const title = typeof jobOrTitle === 'string' ? jobOrTitle : jobOrTitle?.title;
        return /regional medical partner/i.test(title || '');
    }

    function resolvePetfolkHospitalName(title = '', location = '') {
        void title;
        void location;
        return PETFOLK_HOSPITAL_NAME;
    }

    function getPetfolkLocationHospitalSuffix(location) {
        const parsed = typeof location === 'string' ? parseLocationValue(location) : location;
        const rawLocation = typeof location === 'string'
            ? location
            : (parsed?.location || parsed?.city || '');
        const city = parsed?.city || String(rawLocation || '').split(',')[0];
        const suffix = (city || rawLocation || '')
            .replace(/\([^)]*\)/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();

        if (!suffix || /^hybrid$/i.test(suffix)) return '';
        return suffix;
    }

    function getPetfolkHospitalNameForLocation(location) {
        const suffix = getPetfolkLocationHospitalSuffix(location);
        return suffix ? `${PETFOLK_HOSPITAL_NAME} - ${suffix}` : PETFOLK_HOSPITAL_NAME;
    }

    function isPetfolkExtractedChildJob(job) {
        const isLegacyGeneratedChild = !!(
            job?.isNewLocation &&
            job?.sourceLink &&
            isPetfolkJob(job)
        );
        return !!(job?.locationExtractedChild || /-LOC\d+$/i.test(job?.jobId || '') || isLegacyGeneratedChild);
    }

    function isLocationExtractionHighlightRow(job) {
        return !!(job?.locationExtractedParent || isPetfolkExtractedChildJob(job));
    }

    function normalizePetfolkDescriptionWhitespace(value = '') {
        return (value || '')
            .replace(/\r/g, '')
            .replace(/[ \t]+/g, ' ')
            .replace(/\n[ \t]+/g, '\n')
    ...
    function getPetfolkDescriptionBody(description = '') {
        const marker = description.match(/=== DESCRIPTION & REQUIREMENTS ===\s*/i);
        return marker ? description.slice(marker.index + marker[0].length) : description;
    }

    function getLeadingPetfolkTitle(text = '') {
        const firstLine = (text || '').split(/\r?\n/).map(line => line.trim()).find(Boolean) || '';
        if (!firstLine) return '';

        const normalizedFirstLine = normalizeSimpleText(firstLine);
        const isKnownPosition = APPROVED_POSITIONS.some(position => normalizeSimpleText(position) === normalizedFirstLine);
        return isKnownPosition ? firstLine : '';
    }

    function removeLeadingPetfolkTitle(text = '', title = '') {
        const lines = (text || '').split(/\r?\n/);
        ...
        const normalizedFirstLine = normalizeSimpleText(firstLine);
        const normalizedTitle = normalizeSimpleText(title);
        const isRequestedTitle = normalizedTitle && normalizedFirstLine === normalizedTitle;
        const isKnownPosition = APPROVED_POSITIONS.some(position => normalizeSimpleText(position) === normalizedFirstLine);

        if (!isRequestedTitle && !isKnownPosition) return text;

        lines.splice(firstContentIndex, 1);
        return lines.join('\n').trim();
    }

    function getPetfolkLocationAnnouncement(text = '', selectedLocation = null) {
        const announcement = (text || '')
            .split(/\r?\n/)
            .map(line => line.trim())
            .find(line => /^Petfolk\s+.+?\s+-\s+Coming soon\s+\d{4}!$/i.test(line));

        if (!announcement) return '';

        const city = selectedLocation?.city || '';
        if (!city) return announcement;

        return announcement.replace(/^Petfolk\s+.+?\s+-\s+/i, `Petfolk ${city} - `);
    }

    function removePetfolkLocationAnnouncements(text = '') {
        return (text || '')
            .replace(/^\s*Petfolk\s+.+?\s+-\s+Coming soon\s+\d{4}!\s*$/gim, '')
            .replace(/\n{3,}/g, '\n\n')
            .trim();
    }

    function inferStateAbbreviationFromText(text = '', fallbackLocation = null) {
        if (fallbackLocation?.stateAbbr) return fallbackLocation.stateAbbr;
        if (fallbackLocation?.state) return getStateAbbreviation(fallbackLocation.state);

        const stateNameMatch = text.match(/\bin the state of ([A-Za-z\s]+?)(?:\.|,|\s+or\b|\s+and\b)/i);
        if (stateNameMatch) {
            const stateAbbr = getStateAbbreviation(stateNameMatch[1].trim());
            if (stateAbbr) return stateAbbr;
        }

        const registrationMatch = text.match(/\bregistration in ([A-Z]{2})\b/i);
        if (registrationMatch) return registrationMatch[1].toUpperCase();
    ...
    function inferPetfolkLocationGroup(locations = []) {
        ...
        const states = [...new Set(locations.map(loc => loc?.stateAbbr || getStateAbbreviation(loc?.state || '')).filter(Boolean))];
        return states.length === 1 ? states[0] : '';
    }

    function formatPetfolkLocationListBlock(locations = []) {
        if (!Array.isArray(locations) || locations.length <= 1) return '';
        const locationLines = locations
            .map(loc => {
                const parsed = typeof loc === 'string' ? parseLocationValue(loc) : loc;
                if (!parsed?.city) return '';
                const stateAbbr = parsed.stateAbbr || getStateAbbreviation(parsed.state || '');
                return stateAbbr ? `${parsed.city}, ${stateAbbr}` : parsed.city;
            })
            .filter(Boolean);
        if (!locationLines.length) return '';
        const group = inferPetfolkLocationGroup(locations);
        return `${locationLines.length}${group ? ` ${group}` : ''} Locations:\n\n${locationLines.join('\n')}`;
    }
    ...
    function preparePetfolkDescriptionText(description = '') {
        return getPetfolkDescriptionBody(description)
            ...
            .replace(/\bDescription\s+(?=As\s+a\s+Petfolk|As\s+an?\s+|Petfolk\s+provides)/gi, '\n\nDescription\n')
            .replace(/(Partner Veterinarian Opportunity|Lead Your Team, Create Your Upside|Ownership Without The Risk|Focus On The Medicine|Part Of A Team|Profit Sharing\. Equity\. Leadership\.)/gi, '\n\n$1\n')
            .replace(/care\.Requirements/gi, 'care.\n\nRequirements\n')
            .replace(/requirementsAdditional Qualifications/gi, 'requirements\n\nAdditional Qualifications\n')
            .replace(/possible\.Benefits/gi, 'possible.\n\nBenefits\n')
            .replace(/servicesWe believe\s*in?/gi, 'services\n\nWe believe in')
            .replace(/\b(Requirements|Additional Qualifications|Benefits)\b/gi, '\n\n$1\n')
            .replace(/\b(Doctorate in Veterinary Medicine|1\+\s+years? in the pet ecosystem|Prioritization of the gold standard|Ability to adapt|A Fear-Free|Passion for improving|Compassionate team player|Life long learner|You love working|You have a knack|You enjoy being|Desirable Compensation|State-of-the-art Modern Facilities|No on-call|Generous Paid Time Off|Up to 100% Covered|100% Covered|Life Insurance|Professional Liability Insurance|Fear Free Veterinary Certification|State Licensure|Annual Professional Development|FIGS Scrub Allowance|Discount on Petfolk services)\b/gi, '\n$1')
            .replace(/\b(We believe\s+in working together)/i, '\n\n$1')
            .replace(/\n{3,}/g, '\n\n')
            .trim();
    }

    function getPetfolkSection(text, heading, nextHeadings = []) {
        const escapedHeading = heading.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const nextPattern = nextHeadings.map(item => item.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
        const pattern = nextPattern
            ? new RegExp(`\\b${escapedHeading}\\b\\s*\\n?([\\s\\S]*?)(?=\\n\\s*(?:${nextPattern})\\b|$)`, 'i')
            : new RegExp(`\\b${escapedHeading}\\b\\s*\\n?([\\s\\S]*)`, 'i');
        const match = text.match(pattern);
        return normalizePetfolkDescriptionWhitespace(match?.[1] || '');
    }

    function formatPetfolkDescriptionSection(text = '') {
        return normalizePetfolkDescriptionWhitespace(text)
            .replace(/\n\s*(Petfolk provides a modern ecosystem)/i, '\n\n$1');
    }
    ...
    function formatPetfolkDescription(description = '', options = {}) {
        if (!description) return '';

        const locations = options.locations || [];
        const selectedLocation = options.selectedLocation || null;
        const stateAbbr = inferStateAbbreviationFromText(description, selectedLocation);
        const prepared = preparePetfolkDescriptionText(description);
        const title = normalizePetfolkDescriptionWhitespace(options.title || getLeadingPetfolkTitle(prepared));
        const withoutTitle = removeLeadingPetfolkTitle(prepared, title);
        const locationAnnouncement = getPetfolkLocationAnnouncement(withoutTitle, selectedLocation);
        const body = removePetfolkLocationAnnouncements(withoutTitle);
        const introMatch = body.match(/\b(At Petfolk,[\s\S]*?care for them\.)/i);
        const intro = normalizePetfolkDescriptionWhitespace(introMatch?.[1] || '');

        let role = '';
        const descriptionMatch = body.match(/\bDescription\b\s*([\s\S]*?)(?=\n\s*(?:Requirements)\b|$)/i);
        if (descriptionMatch) {
            role = descriptionMatch[1];
        } else {
            const afterIntro = intro ? body.slice(body.indexOf(intro) + intro.length) : body;
            role = afterIntro.split(/\n\s*Requirements\b/i)[0] || '';
        }
        ...
        const additional = getPetfolkSection(body, 'Additional Qualifications', ['Benefits', 'We believe']);
        const benefits = getPetfolkSection(body, 'Benefits', ['We believe']);
        const legalMatch = body.match(/\bWe believe\s+in[\s\S]*$/i);
        const legal = normalizePetfolkDescriptionWhitespace(legalMatch?.[0] || '');

        const sections = [];
        if (title) sections.push(title);
        if (intro) sections.push(intro);
        if (locationAnnouncement) sections.push(locationAnnouncement);
        else if (!selectedLocation) {
            const locationBlock = formatPetfolkLocationListBlock(locations);
            if (locationBlock) sections.push(locationBlock);
        }
        if (role) sections.push(`Description\n${formatPetfolkDescriptionSection(role)}`);
        if (requirements) sections.push(`Requirements\n${formatPetfolkListSection(requirements)}`);
        if (additional) sections.push(`Additional Qualifications\n${formatPetfolkListSection(additional)}`);
        if (benefits) sections.push(`Benefits\n${formatPetfolkListSection(benefits)}`);
        if (legal) sections.push(legal);

        return normalizePetfolkDescriptionWhitespace(sections.join('\n\n') || getPetfolkDescriptionBody(description));
    }

    function localizeDescriptionForLocation(description = '', location, options = {}) {
        if (!description) return '';

        if (!location?.location) {
            return formatPetfolkDescription(description, {
                locations: extractLocationsFromText(description),
                title: options.title || ''
            });
        }

        let localized = description.replace(
            /Locations:\s*\n(?:\s*-\s*[^\n]+\n?)+/i,
            `Locations:\n  - ${location.location}\n`
        );
        ...
        localized = localized.replace(/^Selected Location:.*$/m, '').trim();
        return formatPetfolkDescription(localized, {
    ...
    function getSelectedPetfolkLocationForJob(job, locations = []) {
        const parsedLocation = parseLocationValue(job?.location || '');
        if (!parsedLocation?.location) return null;

        if (job?.locationsExtracted || job?.locationExtractedChild || locations.length <= 1) {
    ...
    function getFormattedDescriptionForJob(job) {
        if (!job?.description) return '';
        if (!isPetfolkJob(job)) return job.description;

        const locations = getJobLocations(job);
        return formatPetfolkDescription(job.description, {
            title: getPetfolkDescriptionTitleForJob(job),
            locations,
            selectedLocation: getSelectedPetfolkLocationForJob(job, locations)
        });
    }

    function analyzePetfolkJob(job, locationOverride = null) {
        const title = job.title || '';
        const sourceDescription = getPetfolkSourceDescription(job);
        const description = sourceDescription || job.description || '';
        const locations = getJobLocations(job);
        const selectedRowLocation = getSelectedPetfolkLocationForJob(job, locations);
        const currentRowLocation = /\|/.test(job.location || '') ? null : parseLocationValue(job.location || '');
        ...
        return {
            areaOfPractice: determinePetfolkAreaOfPractice(title, description),
            position,
            salary: extractPetfolkSalary(description),
            hospitalName: getPetfolkHospitalNameForLocation(location),
            jobType: extractPetfolkJobType(title, description),
            experience: extractPetfolkExperience(description),
    ...
    function extractDetailsFromDescription(positionTitle, descriptionText) {
        ...
        function determineAreaOfPractice(positionText, descriptionText, hospitalName = '') {
            ...
            if (hasUrgentCareAreaSignal(positionText, descriptionText, `${hospitalName || ''} ${department}`)) return 'Urgent Care';
            ...
            if (department) {
                ...
                if (/\bemergency\b/.test(department) || /\b(?:er|e\/r)\b/.test(department)) return 'Emergency Care';
            ...
            if (hasEmergencySignal(title, hospitalName, department) || title.includes('ecc')) {
    ...
    function matchesSearch(job, term) {
        return (
            ...
            getHospitalNameForJob(job).toLowerCase().includes(term) ||
    ...
    function openDescriptionModal(job) {
        if (!job?.description) {
            showToast('No saved description found for this job.', 'error');
            return;
        }

        descriptionModalTitle.textContent = job.title || 'Job Description';
        descriptionModalMeta.textContent = [getDisplayJobId(job), job.location || '']
            .filter(Boolean)
            .join(' - ') || 'Saved description for the selected job.';
        descriptionModalContent.textContent = getFormattedDescriptionForJob(job);
    ...
    function closeDescriptionModal() {
        ...
        descriptionModalMeta.textContent = 'Saved description for the selected job.';
        descriptionModalContent.textContent = '';
    }
    ...
    function openEditJobModal() {
        ...
        editJobMeta.textContent = `${getDisplayJobId(job) || 'No Job ID'} - ${job.title || 'Untitled Job'}`;
        ...
        fieldIds.forEach(field => {
            ...
            if (input) {
                if (field === 'description') {
                    input.value = getFormattedDescriptionForJob(job);
                } else if (field === 'hospital') {
                    input.value = getHospitalNameForJob(job);
                } else {
    ...
    async function saveEditedJob(event) {
        ...
        normalizePetfolkJobHospital(allJobs[jobIndex]);
    ...
    function displayRecords(jobs) {
        ...
        jobs.forEach((job, index) => {
            ...
            // Mark only multi-location parent rows and the child rows created from them.
            if (isLocationExtractionHighlightRow(job)) {
                row.classList.add('location-extracted-row');
            }
            ...
            jobIdCell.textContent = getDisplayJobId(job) || 'N/A';
            jobIdCell.style.fontFamily = "'Consolas', 'Monaco', monospace";
            jobIdCell.style.fontSize = '12px';
            jobIdCell.style.color = '#64748b';
            row.insertCell(4).textContent = getHospitalNameForJob(job);
            row.insertCell(5).textContent = 'Petfolk (Parent Client)';
            ...
            if (job.description) {
                ...
                viewDescriptionButton.type = 'button';
                viewDescriptionButton.className = 'description-view-btn';
                viewDescriptionButton.textContent = 'View Description';
                viewDescriptionButton.addEventListener('click', () => {
                    openDescriptionModal(job);
                });
                descCell.appendChild(viewDescriptionButton);
            } else {
    ...
    function exportToCSV() {
        ...
        const csvContent = [
            ...
            ...allJobs.map((job, index) => [
                index + 1,
                `"${(job.title || '').replace(/"/g, '""')}"`,
                `"${getDisplayJobId(job).replace(/"/g, '""')}"`,
                `"${getHospitalNameForJob(job).replace(/"/g, '""')}"`,
                `"Petfolk (Parent Client)"`,
                ...
                `"${(job.link || '').replace(/"/g, '""')}"`,
                `"${getFormattedDescriptionForJob(job).replace(/"/g, '""')}"`
            ].join(','))
        ].join('\n');
        ...
        link.setAttribute('download', `petfolk_jobs_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    sendToWebhookButton.addEventListener('click', async () => {
        ...
        const jobsToSend = jobs.map(job => ({
            job_title: job.title,
            job_id: getDisplayJobId(job),
            department_id: getDisplayJobId(job),
            hospital: getHospitalNameForJob(job),
            aggregator: "Petfolk (Parent Client)",
            street_address: job.streetAddress || '',
            parent_client: "Petfolk (Parent Client)",
            city: job.city,
            state: job.state,
            ...
            url: job.link,
            link: job.link,
            description: getFormattedDescriptionForJob(job)
        }));
        ...
        for (let i = 0; i < totalBatches; i++) {
            ...
            const payload = {
                source: 'Petfolk Job Scraper',
                parentClientName: 'Petfolk (Parent Client)',
                syncId: syncId,
    ...
    function buildDescriptionPageUrl(link) {
        const jobUrl = new URL(link);
        if (jobUrl.hostname.includes('jobvite.com')) {
            jobUrl.searchParams.set('nl', '1');
        }
        return jobUrl.toString();
    }

    function findQueuedJobIndex(jobs, queueItem) {
        if (!queueItem) return -1;

        if (queueItem.jobId) {
            const jobIdMatch = jobs.findIndex(job => job.jobId === queueItem.jobId);
            if (jobIdMatch !== -1) return jobIdMatch;
        }

        if (Number.isInteger(queueItem.index)) {
            const indexedJob = jobs[queueItem.index];
            const sameJobId = !queueItem.jobId || indexedJob?.jobId === queueItem.jobId;
            const sameLink = !queueItem.link || indexedJob?.link === queueItem.link;
            if (indexedJob && sameJobId && sameLink) return queueItem.index;
        }
    ...
    function needsDescriptionScrape(job) {
    ...
    async function saveDescriptionForQueueItem(description, queueItem, fallbackJobIndex) {
        const data = await chrome.storage.local.get(['scrapedJobs']);
        const jobs = data.scrapedJobs || [];
        let jobIndex = findQueuedJobIndex(jobs, queueItem);

        if (jobIndex === -1 && Number.isInteger(fallbackJobIndex) && jobs[fallbackJobIndex]) {
            jobIndex = fallbackJobIndex;
        }

        if (jobIndex === -1 || !jobs[jobIndex]) {
            throw new Error('Job was not found in saved records.');
        }

        const targetJob = jobs[jobIndex];
        const targetSourceId = targetJob.sourceJobId || '';
        const targetLink = targetJob.link || queueItem.link || '';
    ...
    async function processNextDetail() {
        ...
        if (positionTitle && isPetfolkJob(job)) {
            detailsList = [analyzePetfolkJob(job)];
        ...
        return new Promise((resolve) => {
            chrome.storage.local.get(['scrapedJobs'], (data) => {
                ...
                if (isPetfolkJob(originalJob)) {
                    ...
                    // Fetch Details should not change the row identity. Extract Locations
                    // owns hospital/city/state/location creation for Petfolk parent/child rows.
                    if (!originalJob.hospital) originalJob.hospital = firstDetail.hospitalName || '';
                    if (!originalJob.city) originalJob.city = formatCityForStorage(firstDetail.city || '');
                    if (!originalJob.state) originalJob.state = formatStateForStorage(firstDetail.state || '');
                    if (!originalJob.location) originalJob.location = firstDetail.location || '';

                    if (firstDetail.description) {
                        originalJob.description = firstDetail.description;
                    }
                    if (firstDetail.sourceDescription) {
                        originalJob.petfolkSourceDescription = firstDetail.sourceDescription;
                    }
    ...
    function getPetfolkLocationKey(job, location) {
        const sourceId = job.sourceJobId || extractRipplingJobIdFromLink(job.link || job.sourceLink || '') || getDisplayJobId(job);
        return `${normalizeSimpleText(sourceId)}|${normalizeSimpleText(location?.location || job.location || '')}`;
    }

    function getPetfolkChildJobId(parentJob, index) {
        const sourceId = parentJob.sourceJobId || extractRipplingJobIdFromLink(parentJob.link || parentJob.sourceLink || '') || getDisplayJobId(parentJob).replace(/^PFV-/i, '');
        const cleanSourceId = String(sourceId || '')
            .replace(/^PFV-/i, '')
            .replace(/^PETFOLK-/i, '')
            .replace(/[^a-z0-9-]+/gi, '-')
            .replace(/^-+|-+$/g, '');
        return `PFV-${cleanSourceId || 'LOCATION'}-LOC${index + 1}`;
    }

    function getPetfolkLocationDetails(job, location, sourceDescription = '') {
        const selectedLocation = location || getJobLocations(job)[0] || parseLocationValue(job.location || '');
        const title = getPetfolkDescriptionTitleForJob(job);
        const description = sourceDescription || getPetfolkSourceDescription(job);
        return {
            hospitalName: getPetfolkHospitalNameForLocation(selectedLocation),
            city: selectedLocation?.city || job.city || '',
            state: selectedLocation?.state || job.state || '',
            location: selectedLocation?.location || job.location || '',
            description: localizeDescriptionForLocation(description, selectedLocation, { title })
        };
    }

    function applyPetfolkLocationDetails(job, details, sourceLocations, sourceDescription = '') {
        const normalizedSourceLocations = Array.isArray(sourceLocations) ? sourceLocations : [];
        job.hospital = details.hospitalName || job.hospital || '';
        job.city = formatCityForStorage(details.city || job.city || '');
        job.state = formatStateForStorage(details.state || job.state || '');
        job.location = details.location || job.location || '';
        if (details.description) {
            job.description = details.description;
        }
        if (sourceDescription) {
            job.petfolkSourceDescription = sourceDescription;
        }
        job.petfolkSourceLocations = normalizedSourceLocations.map(loc => loc.location);
        job.locations = normalizedSourceLocations.map(loc => loc.location);
        job.allLocations = normalizedSourceLocations.map(loc => loc.location).join(' | ');

        if (job.locationExtractedChild) {
            delete job.locationExtractedParent;
            job.locationsExtracted = true;
            job.isNewLocation = true;
        } else {
            delete job.locationExtractedParent;
            delete job.locationsExtracted;
            delete job.isNewLocation;
        }
    }

    async function extractLocationsForSavedJobs() {
        if (!extractLocationsBtn) return;
        if (isGettingDescriptions || isFetchingDetails || isFetchingAddresses) {
            showToast('Please wait for the current action to finish first.', 'error');
            return;
        }
        ...
        extractLocationsBtn.disabled = true;
        extractLocationsBtn.textContent = 'Extracting Locations...';

        try {
            const resultJobs = [];
            const existingKeys = new Set();
            const processedPetfolkSourceKeys = new Set();
            let updatedCount = 0;
            let createdCount = 0;
            let skippedCount = 0;

            jobs.forEach(job => {
                if (isPetfolkExtractedChildJob(job)) {
                    skippedCount++;
                    return;
                }
                ...
                if (isRegionalMedicalPartnerJob(job)) {
                    resultJobs.push(job);
                    skippedCount++;
                    return;
                }

                const sourceJobId = job.sourceJobId || extractRipplingJobIdFromLink(job.link || job.sourceLink || '') || getDisplayJobId(job).replace(/^PFV-/i, '').replace(/-LOC\d+$/i, '');
                const sourceKey = normalizeSimpleText(sourceJobId || job.link || job.jobId || '');
                if (sourceKey && processedPetfolkSourceKeys.has(sourceKey)) {
                    skippedCount++;
                    return;
                }
                if (sourceKey) processedPetfolkSourceKeys.add(sourceKey);

                const sourceDescription = getPetfolkSourceDescription(job);
                const extractionSourceJob = {
                    ...job,
                    description: sourceDescription
                };
                const locations = getPetfolkLocationsForExtraction(extractionSourceJob);
                if (!locations.length) {
                    resultJobs.push(job);
                    skippedCount++;
                    return;
                ...
                const baseJob = {
                    ...job,
                    sourceJobId,
                    locationExtractedChild: false,
                    petfolkSourceDescription: sourceDescription
                };
                delete baseJob.locationExtractedChild;
                const firstLocation = locations[0];
                const firstDetails = getPetfolkLocationDetails(baseJob, firstLocation, sourceDescription);
                applyPetfolkLocationDetails(baseJob, firstDetails, locations, sourceDescription);
                resultJobs.push(baseJob);
                updatedCount++;
                existingKeys.add(getPetfolkLocationKey(baseJob, firstLocation));
                let createdChildForParent = false;

                locations.slice(1).forEach((location, index) => {
                    const locationKey = getPetfolkLocationKey(baseJob, location);
                    if (existingKeys.has(locationKey)) return;

                    const childDetails = getPetfolkLocationDetails(baseJob, location, sourceDescription);
                    const childJob = {
                        ...baseJob,
                        jobId: getPetfolkChildJobId(baseJob, index + 1),
                        hospital: childDetails.hospitalName,
                        city: formatCityForStorage(childDetails.city || ''),
                        state: formatStateForStorage(childDetails.state || ''),
                        location: childDetails.location || '',
                        streetAddress: '',
                        zipCode: '',
                        sourceLink: job.link || job.sourceLink || '',
                        sourceJobId,
                        petfolkSourceDescription: sourceDescription,
                        isNewLocation: true,
                        locationExtractedChild: true
                    };
                    applyPetfolkLocationDetails(childJob, childDetails, locations, sourceDescription);
                    resultJobs.push(childJob);
                    existingKeys.add(locationKey);
                    createdCount++;
                    createdChildForParent = true;
                });

                if (createdChildForParent) {
                    baseJob.locationExtractedParent = true;
                    baseJob.locationsExtracted = true;
                }
            });

            await chrome.storage.local.set({ scrapedJobs: resultJobs });
            allJobs = resultJobs;
            selectedJobKeys.clear();
            renderCurrentView();
            showToast(`Locations extracted. Updated ${updatedCount}, created ${createdCount}, skipped ${skippedCount}.`, 'success');
        } catch (error) {
            console.error('Error extracting Petfolk locations:', error);
            showToast('Error extracting locations. Please check console.', 'error');
        } finally {
            extractLocationsBtn.disabled = false;
            extractLocationsBtn.innerHTML = `
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
    ...
    if (extractLocationsBtn) {
        extractLocationsBtn.addEventListener('click', extractLocationsForSavedJobs);
    }
    ...
    function savedAddressBrandMismatch(job) {
        if (isPetfolkExpectedJob(job)) {
            const hasAddressData = !!(job.streetAddress || job.zipCode || job.website || job.phone);
            if (!hasAddressData) return false;
            return !isPetfolkHospitalNameValue(job.hospital || '') || !isPetfolkWebsite(resolveWebsiteForHospital(job.hospital || '', job.website || ''));
        }
    ...
    fetchAddressesBtn.addEventListener('click', async () => {
        ...
        jobs.forEach(job => {
            if (isPetfolkExpectedJob(job) && !isPetfolkHospitalNameValue(job.hospital || '')) {
                job.hospital = getPetfolkHospitalNameForLocation(parseLocationValue(job.location || ''));
            }
    ...
    async function processNextAddress() {
        ...
        try {
            ...
            const isPetfolkAddressJob = isPetfolkExpectedJob(job);
            const petfolkOfficialAddressData = isPetfolkAddressJob
                ? await fetchPetfolkOfficialAddressForJob(job)
                : null;
            let addressData = petfolkOfficialAddressData
                ? { ...petfolkOfficialAddressData }
                : lookupTarget.directResult
                    ? { ...lookupTarget.directResult }
                    : null;
            const shouldSkipCache = !!petfolkOfficialAddressData || !!lookupTarget.directResult || isMissionPetHealthHospital(job.hospital);
            if (!shouldSkipCache) addressData = getRememberedAddress(cacheKeys);

            if (addressData) {
                const sourceLabel = petfolkOfficialAddressData
                    ? 'Using Petfolk official'
                    : lookupTarget.directResult
                        ? 'Using override'
                        : 'Using cached';
            ...
            const isOfficialPetfolkAddress = !!addressData?.isPetfolkOfficial;
            const hasExactFetchedCityState =
                !!(searchCity && searchState && fetchedCity && fetchedState) &&
                formatCityForStorage(fetchedCity) === formatCityForStorage(searchCity) &&
                getStateAbbreviation(fetchedState) === getStateAbbreviation(searchState);
            const fetchedLocationMismatch =
                !isOfficialPetfolkAddress && (
            ...
            if (jobs[index]) {
                ...
                const resolvedCity = isOfficialPetfolkAddress
                    ? addressData.city
                    : searchCity || addressData.city || jobs[index].city || '';
                const resolvedState = isOfficialPetfolkAddress
                    ? addressData.state
                    : searchState || addressData.state || jobs[index].state || '';
                jobs[index].city = formatCityForStorage(resolvedCity || '');
                jobs[index].state = formatStateForStorage(resolvedState || '');
                if (isOfficialPetfolkAddress && resolvedCity && resolvedState) {
                    const stateAbbr = getStateAbbreviation(resolvedState) || resolvedState;
                    jobs[index].location = `${formatCityForStorage(resolvedCity)}, ${stateAbbr}`;
                }

                if ((isOfficialPetfolkAddress || hasExactFetchedCityState) && addressData.businessName) {
                    jobs[index].hospital = addressData.businessName;
                }
