...
(async () => {
    try {
        ...
        while (Date.now() - startTime < MAX_WAIT) {
            ...
            // If no results yet, try to trigger search by clicking search box
            if (Date.now() - startTime > 3000 && resultLinks.length === 0) {
                tryTriggerSearch();
            }
    ...
    // ===== Try to trigger search if search box is available =====
    function tryTriggerSearch() {
        try {
            // Look for the search input box
