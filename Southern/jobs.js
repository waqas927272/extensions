// Shore Capital Job Scraper - Jobs Page Script
...
function populateCompanyFilter() {
  ...
  const companies = [...new Set(allJobs.map(job => job.hospitalName))].sort();
...
function setupEventListeners() {
  ...
  // No data button
  const goCompaniesBtn = document.getElementById('go-companies-btn');
  if (goCompaniesBtn) {
...
function filterJobs() {
  ...
  filteredJobs = allJobs.filter((job) => {
    ...
    // Company filter
    const matchesCompany = companyFilter === 'all' || job.hospitalName === companyFilter;
  ...
  filteredJobs.forEach((job, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="col-company">
        <span class="company-badge">${escapeHtml(job.hospitalName)}</span>
      </td>
...
function updateStats() {
  const uniqueCompanies = new Set(allJobs.map(j => j.hospitalName)).size;
...
function exportCSV() {
  ...
  const headers = ['Job Title', 'Company', 'City', 'State', 'Country', 'Link', 'Description'];
...
function showJobDetails(job) {
  const location = [job.city, job.state, job.country].filter(Boolean).join(', ') || 'N/A';

  document.getElementById('modal-title').textContent = job.title;
  document.getElementById('modal-body').innerHTML = `
    <p><strong>Company:</strong> ${escapeHtml(job.hospitalName)}</p>
