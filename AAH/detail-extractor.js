(() => {
    ...
    function determineAreaOfPractice(title, category, descriptionText) {
        ...
        if (aopFromCategory === 'Specialty Care') return aopFromCategory;

        // Board-certified or residency-trained roles are specialty roles even if Jobvite category is broad.
        if (/\bboard[\s-]+certified\b/i.test(descriptionText) ||
            /\bresidenc(?:y|e)[\s-]+trained\b/i.test(descriptionText)) {
    ...
    function formatSalary(raw, context = '') {
        ...
        // Check if it's hourly using both the matched salary and nearby text.
