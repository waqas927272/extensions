...
async function getJobDescriptions() {
    ...
    for (const job of jobsToScrape) {
        ...
        try {
            let descriptionScript = 'description-scraper.js';
            if (job.link && job.link.includes('greenhouse.io')) {
                descriptionScript = 'greenhouse-description-scraper.js';
            }

            const description = await scrapeDescriptionFromPage(job.link, descriptionScript);
...
function scrapeDescriptionFromPage(url, scriptName) {
            ...
            const listener = async (tabId, changeInfo, updatedTab) => {
                if (tabId === tab.id && changeInfo.status === 'complete') {
                    ...
                    if (results && results[0] && results[0].result) {
                        ...
                        reject(new Error(`Script (${scriptName}) did not return a result.`));
                    }
                }
            };
...
function exportToJSON() {
    chrome.storage.local.get(['scrapedJobs'], (data) => {
        ...
        downloadFile(jsonContent, 'vetpractice-jobs.json', 'application/json');
    });
}
