document.addEventListener('DOMContentLoaded', () => {
  const recordsTableBody = document.querySelector('#jobRecordsTable tbody'); // Adapted ID
  ...
  let allRecords = []; // This will now hold jobs with description/hospitalName
  ...
  function loadRecords() {
    chrome.storage.local.get({ scrapedJobs: [] }, (result) => { // Adapted storage key
      allRecords = result.scrapedJobs; // Adapted storage key
  ...
  function renderTable(records) {
    ...
    records.forEach(record => {
      ...
      row.innerHTML = `
        <td>${record.title}</td>
        <td>${record.hospital || 'N/A'}</td> <!-- Adapted field -->
        <td>${record.state}</td>
        <td>${record.country || 'N/A'}</td> <!-- Adapted field -->
  ...
  clearRecordsButton.addEventListener('click', () => {
    chrome.storage.local.set({ scrapedJobs: [] }, () => { // Adapted storage key
  ...
  downloadCsvButton.addEventListener('click', () => {
    ...
    const headers = ['Job Title', 'Hospital', 'State', 'Country', 'Link', 'Description']; // Adapted headers
    let csvContent = headers.join(',') + '\n';

    allRecords.forEach(record => {
      const escapeCsvCell = (cell) => {
        const strCell = String(cell);
        if (strCell.includes(',')) { // Fix: Changed StrStr to strCell
          return `"${strCell}"`;
        }
        return strCell;
      };
      ...
      const row = [
        ...
        escapeCsvCell(record.hospital || 'N/A'), // Adapted field
        escapeCsvCell(record.state),
        escapeCsvCell(record.country || 'N/A'), // Adapted field
        escapeCsvCell(record.link),
        escapeCsvCell(record.description || 'N/A') // Always include description in CSV
      ].join(',');
      csvContent += row + '\n';
    });
  ...
  getJobDescriptionsButton.addEventListener('click', async () => {
    ...
    chrome.storage.local.set({ scrapedJobs: allRecords }, () => { // Adapted storage key
